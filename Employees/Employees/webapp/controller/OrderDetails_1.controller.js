...
sap.ui.define([
    ...
    function _readSignature(orderId, employeeId) {
        ...
        this.byId("uploadCollection").bindAggregation("items", {
            ...
            filters: [
                new Filter("OrderId", FilterOperator.EmployeeIDEQ, orderId),
        ...
        factoryOrderDetails: function (listId, oContext) {
            ...
            if (contextObject.Quantity <= unitsInStock) {
                var objectListItem = new sap.m.ObjectListItem({
                    title: "{odataNorthwind>/Products(" + contextObject.
                        ProductID + ")/ProductName} ({odataNorthwind>Quantity})",
                ...
                var customListItem = new sap.m.CustomListItem({
                    content: [
                        new sap.m.Bar({
                            contentLeft: new sap.m.Label({ text: "{odataNorthwind>/Products(" + contextObject.ProductID + ")/ProductName} ({odataNorthwind>Quantity})" }), contentMiddle: new sap.m.ObjectStatus({ text: "{i18n>availableStock} {odataNorthwind>/Products(" + contextObject.ProductID + ")/UnitsInStock}", state: "Error" }),
